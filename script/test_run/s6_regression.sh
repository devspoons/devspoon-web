...
for f in compose/web-service/nginx_{gunicorn,uvicorn,uwsgi,daphne}/docker-compose.yml; do
...
if grep -q 'ensure_env_secrets()' script/lib/django_secrets.sh; then
    ...
    cp compose/web-service/nginx_gunicorn/docker-compose.yml "$tmp/dj/"; cp compose/web-service/nginx_php-8.4/docker-compose.yml "$tmp/php/"
...
assert_zero "6.24 .gitignore 옛 compose/web_service 경로 (CL-WP5-04 TC-W15-1)" "$(grep -c web_service .gitignore)"
...
for d in compose/web-service/nginx_*; do
...
for s in gunicorn uvicorn uwsgi daphne php-7.3 php-8.4; do
    assert_zero "6.26 고정 devspoon 이미지명 ($s)" "$(grep -cE '^[[:space:]]+image: devspoon-' compose/web-service/nginx_$s/docker-compose.yml)"
done
for f in script/test_run/verify_integration_*.sh script/test_run/verify_healthcheck.sh script/test/verify-ngxblocker.sh; do
...
assert_zero "6.28 생성기의 compose/web-service 고정 경로" "$(cat config/web-server/nginx/*/nginx_http*_conf.sh | grep -c 'compose/web-service')"
for s in gunicorn uvicorn uwsgi daphne; do
...
EOF
    ...
    echo "  FAIL 6.35 $g 판정 함수(logrotate_dry_ok·lock_version·env_missing) 없음"; FAILS=$((FAILS+1))
fi
echo
