#!/usr/bin/env bash
# Integration test for nginx_php-8.4 stack — 출고 conf.d 그대로 기동해 단언한다. 실패 시 exit 1.
set -uo pipefail
...
check "봇 UA 차단 (exit 52·444)"     'http_blocked -A MJ12bot -H "Host: localhost" http://127.0.0.1/'
