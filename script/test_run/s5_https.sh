...
trap 'rm -f "$NGINX_DIR/conf.d/s5_${STACK}_ng_https.conf"; (cd "$ROOT/compose/web-service/$STACK_DIR" && docker compose exec -T webserver sh -c "rm -rf /etc/letsencrypt/live/$DOM; nginx -s reload" >/dev/null 2>&1)' EXIT
